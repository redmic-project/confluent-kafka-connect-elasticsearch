...
public class DataConverterTest {
	...
	public void array() {
		...
		assertEquals(Arrays.asList(0.02, 0.42), converter.preProcessValue(
				Arrays.asList(new BigDecimal("0.02"), new BigDecimal("0.42")), origSchema, preProcessedSchema));
	...
	public void map() {
		...
		Schema preProcessedSchema = converter.preProcessSchema(origSchema);
		assertEquals(SchemaBuilder
				.array(SchemaBuilder.struct().name(Schema.INT32_SCHEMA.type().name() + "-" + Decimal.LOGICAL_NAME)
						.field(ElasticsearchSinkConnectorConstants.MAP_KEY, Schema.INT32_SCHEMA)
						.field(ElasticsearchSinkConnectorConstants.MAP_VALUE, Schema.FLOAT64_SCHEMA).build())
				.build(), preProcessedSchema);
		...
		assertEquals(
				new HashSet<>(Arrays.asList(
						new Struct(preProcessedSchema.valueSchema()).put(ElasticsearchSinkConnectorConstants.MAP_KEY, 1)
								.put(ElasticsearchSinkConnectorConstants.MAP_VALUE, 0.02),
						new Struct(preProcessedSchema.valueSchema()).put(ElasticsearchSinkConnectorConstants.MAP_KEY, 2)
		...
		assertEquals(
				...
				converter.preProcessSchema(SchemaBuilder.map(Schema.INT32_SCHEMA, Decimal.schema(2))
						.defaultValue(Collections.emptyMap()).build()));
	}
		...
		assertEquals(
				SchemaBuilder
						.array(SchemaBuilder.struct()
								...
								.field(ElasticsearchSinkConnectorConstants.MAP_VALUE, Schema.INT32_SCHEMA).build())
						.build(),
				preProcessedSchema);
		assertEquals(new HashSet<>(Arrays.asList(
				new Struct(preProcessedSchema.valueSchema()).put(ElasticsearchSinkConnectorConstants.MAP_KEY, "field1")
						.put(ElasticsearchSinkConnectorConstants.MAP_VALUE, 1),
				new Struct(preProcessedSchema.valueSchema()).put(ElasticsearchSinkConnectorConstants.MAP_KEY, "field2")
	...
	@Test
	public void geoJsonPoint() {

		Schema propertiesSchema = SchemaBuilder.struct().field("date", Schema.INT64_SCHEMA)
				.field("inserted", Schema.OPTIONAL_FLOAT64_SCHEMA).field("updated", Schema.OPTIONAL_FLOAT64_SCHEMA)
				.build();

		Schema origSchema = SchemaBuilder.struct().name("struct").field("geometry", Schema.STRING_SCHEMA)
				.field("properties", propertiesSchema).build();

		Schema preProcessedSchema = converter.preProcessSchema(origSchema);

		assertNotEquals(origSchema, preProcessedSchema);

		Struct result = (Struct) converter.preProcessValue(
				new Struct(origSchema).put("geometry", "{\"type\": \"Point\", \"coordinates\": [0.02, 0.42]}")
						.put("properties", new Struct(propertiesSchema).put("date", 1545986513L)),
				origSchema, preProcessedSchema);

		Struct expected = new Struct(preProcessedSchema).put("geometry",
				new Struct(preProcessedSchema.field("geometry").schema()).put("type", "Point").put("coordinates",
						Arrays.asList(0.02, 0.42)))
				.put("properties", new Struct(preProcessedSchema.field("properties").schema())
						.put("date", new DateTime(1545986513, DateTimeZone.UTC).toString(DataConverter.dateFormat))
						.put("inserted", result.getStruct("properties").getString("inserted"))
						.put("updated", result.getStruct("properties").getString("updated")));
	...
	private void testOptionalFieldWithoutDefault(SchemaBuilder optionalFieldSchema) {
		Schema origSchema = SchemaBuilder.struct().name("struct")
				.field("optionalField", optionalFieldSchema.optional().build()).build();
		Schema preProcessedSchema = converter.preProcessSchema(origSchema);

		Object preProcessedValue = converter.preProcessValue(new Struct(origSchema).put("optionalField", null),
				origSchema, preProcessedSchema);
